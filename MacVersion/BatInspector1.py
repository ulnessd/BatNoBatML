...
def resource_path(*parts: str) -> Path:
    """
    Resolve a data/asset path that works when running from source or when frozen by PyInstaller.
    - one-folder: assets live next to the executable (dist/YourApp/…)
    - one-file: assets are unpacked to sys._MEIPASS
    """
...
ICON_FILE = resource_path('BatSherlockHolms.png')
...
class MainWindow(QWidget):
    ...
    def _show_startup_placeholder(self):
        ...
        try:
            ...
            img_path = resource_path("BatSherlockHolms.png")
    ...
    def _export_video_mapped(self):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tdir:
                ...
                # Get the full, absolute path to the ffmpeg executable
                ffmpeg_exe = imageio_ffmpeg.get_ffmpeg_exe()

                # 4) Mux into MOV with ffmpeg (copy video, AAC audio, faststart)
                cmd = [
                    ffmpeg_exe,  # Use the full path instead of just "ffmpeg"
                    "-y",
                    "-i", str(silent_video),
                    "-i", str(aud_wav),
                    "-c:v", "copy",
